...
db = cluster.DBSCAN(eps=0.3, min_samples=40).fit(data_without_labels)
...
def find_best_R(R, df_dist, test_df):
	...
	sum_inside_R_4[max_index]/(sum_inside_R_4[max_index]+sum_inside_R_oth[max_index]))
	print ('Didziausias spindulys, kokiame galima rasti tuos rutuliukus: ', R[difference_for_min])
	print ('Control call: sum of melonoma data', sum_inside_R_4[-1])
	print ('Control call: sum of other label data', sum_inside_R_oth[-1])
